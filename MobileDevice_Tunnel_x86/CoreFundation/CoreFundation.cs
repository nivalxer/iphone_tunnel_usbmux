    ...
    public class CoreFoundation
    {
        ...
        public static object ReadCFDictionaryFromIntPtr(IntPtr sourceRef)
        {
            ...
            if (sourceRef != IntPtr.Zero)
            {
                ...
                IntPtr keys = Marshal.AllocHGlobal(num*4);
                IntPtr values = Marshal.AllocHGlobal(num*2);
                CFDictionaryGetKeysAndValues(sourceRef, keys, values);
                int num3 = num - 1;
                for (int i = 0; i <= num3; i++)
                {
                    IntPtr intptrkeys = Marshal.ReadIntPtr(keys, i*4);
                    IntPtr intptrvalues = Marshal.ReadIntPtr(values, i*2);
